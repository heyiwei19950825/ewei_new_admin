...
class Nav extends AdminBase
{
    ...
    protected function _initialize()
    {
        ...
        $nav_list        = $this->nav_model->order(['sort' => 'ASC', 'id' => 'ASC'])->where(['s_id'=>$this->admin_id])->select();
    ...
    public function update($id)
    {
        if ($this->request->isPost()) {
            ...
            if ($validate_result !== true) {
                ...
                if ($this->nav_model->allowField(true)->save($data, ['id'=>$id,'s_id'=>$this->admin_id]) !== false) {
                    $this->success('更新成功');
                } else {
                    $this->error('更新失败');
                }
    ...
    {
        if ($this->nav_model->destroy(['id'=>$id,'s_id'=>$this->admin_id])) {
            $this->success('删除成功');
        } else {
            $this->error('删除失败');
        }
