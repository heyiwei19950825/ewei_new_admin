...
class Menu extends AdminBase
{
    ...
    protected function _initialize()
    {
        ...
        $admin_menu_list       = $this->auth_rule_model->alias('a')->join('module m','a.plug_in = m.id','LEFT')->field('m.module_name,a.*')->order(['a.sort' => 'DESC', 'a.id' => 'ASC'])->select();
        $admin_menu_level_list = array2level($admin_menu_list);
        $module_list           = Db::name('module')->where(['status'=>1])->field('id,module_name')->select();
